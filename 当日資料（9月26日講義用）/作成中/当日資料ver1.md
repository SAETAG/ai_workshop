...
レビューダッシュボードが完成したら、次はあなたのアイデアを形にしてみましょう！Create.xyz を使えば、プログラミング不要で、作りたいアプリを AI と対話しながら作れます。

**アイデアを出すヒント**

- **AI に相談する**: 「Create.xyz で簡単に作れるアプリのアイデアを 5 つ教えて」と聞いてみましょう。
- **身近な課題を解決する**: 毎日のちょっとした不便を解決するツール（例：ラーメンタイマー、買い物リスト）を考えてみましょう。
- **佐賀の魅力を発信する**: 佐賀県に関するクイズアプリや、観光スポットガチャなど、地域をテーマにしたアプリも面白いでしょう。

AI と一緒にプロンプトを作成し、あなただけのオリジナルアプリ作りに挑戦してみてください！

---

## 冒険 3：watsonx.ai ハンズオン：プロジェクト作成からプロンプト実践まで

**推定所要時間**: 30 分

ここからは、watsonx.ai の具体的な操作手順です。アカウントを作れた人は、ぜひ一緒にやってみましょう！
watsonx.ai では、まず作業の入れ物となる「プロジェクト」を作成し、必要な AI サービスを準備する必要があります。
...
- watsonx のホームページで、「新規プロジェクト +」をクリックします。
- 「名前」に `佐賀AIワークショップ` のような分かりやすい名前を入力し、「作成」をクリックします。

---

### 🎮 ステップ 2：Prompt Lab でサンプルプロンプトを試す

次に、いよいよ Prompt Lab を使ってみます。今回は、あらかじめ用意された「サンプル・プロンプト」を元に、佐賀のテーマで AI に質問をしてみましょう。

#### 1. Prompt Lab を開く

- 左上のメニューから watsonx のホーム画面に戻ります。
- 「ファウンデーション・モデルを試し、プロンプトを作成する」をクリックして Prompt Lab を開きます。

#### 2. サンプルプロンプトの選択

- Prompt Lab が開いたら、「サンプル・プロンプト」の中から「**Questions about an article**」を探して選択します。
- これは、記事（Article）の内容について質問（Question）をすると、AI が要約や回答を生成してくれるテンプレートです。
...
① **記事（Article）の入力**
まず、AI に読み込ませる記事として、以下の「吉野ヶ里歴史公園」に関する文章をコピーし、`Article:` の部分に貼り付けてください。
...
③ **AI への質問**
最後に、`Input` の `Question:` に、AI に聞きたい質問を入力します。
...
- AI が記事と例を参考に、質問に対する回答を生成します。`Answer:` の欄に「復元された建物の見学や、勾玉づくりなどの体験プログラム」といった内容の答えが生成されれば成功です。
